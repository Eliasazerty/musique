...
{   
    ...
    $genres_musicaux = $bdd->query("SELECT genre FROM genre_musicaux");
    $sources = $bdd->query("SELECT source FROM sources");
    $output = "";
    if(isset($_POST['submit']))
    {
        if(isset($_POST['titre']) AND isset($_POST['artiste']) AND isset($_POST['album']) AND isset($_POST['genre']) AND isset($_POST['source']) AND isset($_FILES['fichier']))
        {
            if(!empty($_POST['titre']) AND !empty($_POST['artiste']) AND !empty($_POST['album']) AND !empty($_POST['genre']) AND !empty($_POST['source']) AND $_FILES['fichier']['error'] == 0)
            {
                $infos_fichier = pathinfo($_FILES['fichier']['name']);
                $extension_upload = $infos_fichier['extension'];
                $extension_autorisee = array('mp3', 'flac', 'wav');
                if(in_array($extension_upload, $extension_autorisee)) // on vérifie l'extension du fichier
                {
                    ...
                    $req_genre = $bdd->prepare('SELECT id FROM genre_musicaux WHERE genre=?'); // pas de vérification de faites...
                    $req_genre->execute(array($genre));
                    $rep_genre = $req_genre->fetch();

                    $req_source = $bdd->prepare('SELECT id FROM sources WHERE source=?'); // pas de vérification de faites...
                    $req_source->execute(array($source));
                    $rep_source = $req_source->fetch();
                    
                    $genre = $rep_genre['id'];
                    $source = $rep_source['id'];

                    // on upload le fichier audio
                    $nom_du_fichier = basename($_FILES['fichier']['name']);
                    $chemin_du_fichier = 'stockage_musique/'. $nom_du_fichier;
                    move_uploaded_file($_FILES['fichier']['tmp_name'], $chemin_du_fichier);


                    $req = $bdd->prepare('INSERT INTO musiques (titre, artiste, genre, album, source, path, utilisateur) VALUES(:nom, :groupe, :genre, :album, :source, :path, :utilisateur)');
                    $req->execute(array(
                        'nom' => $titre,
                        'groupe' => $artiste,
                        'genre' => $genre,
                        'album' => $album,
                        'source' => $source,
                        'path' => $chemin_du_fichier,
                        'utilisateur' => $_SESSION['id']
                    ));
                    $output = 'la musique '. $titre.' a été enregistrée !';
                }
                else {$output = 'extension du fichier non autorisée !';}
            }
            else {$output= 'un des paramètres du formulaire est vide ou il y a eu une erreur lors de la transmission du fichier (erreur fichier: '. $_FILES['fichier']['error']. ')'; }
        }
        else {$output = 'un des paramètres du formulaire n\'a pas été rempli...';}
    }
    else {$output = "veuillez remplir le formulaire ci-dessus";}
?>
    <!DOCTYPE html>
    <html lang="en">
        <head>
        ...
        <body>
            <form action="ajouter_musique.php" method="post" enctype="multipart/form-data">
                
                <h1>Ajout d'une musique</h1>

                <div class="inputs">
                        <input type="text" name="titre" id="titre" required placeholder="titre">
                        <input type="text" name="artiste" id="artiste" required placeholder="artiste">
                        <input type="text" name="album" id="album" required placeholder="album">
                        
                        <select name="genre" id="genre" required>
                            <?php
                            while($genre_musical = $genres_musicaux->fetch())
                            {
                            ?>
                                <option value="<?php echo $genre_musical['genre']; ?>"><?php echo $genre_musical['genre']; ?></option>
                            <?php
                            }
                            ?>
                        </select>
                ...
                <p id="output" class="output"><?php echo $output; ?></p>
                <div class="bouton">
                    <input type="submit" value="Valider" id="bouton_valider" name="submit" class="button">
                </div>
        ...
        //http://jsfiddle.net/srvikram13/s492L/81/
        //https://github.com/43081j/id3/tree/master/src
...
function readFile(){ // permet de pré-remplir les champs (des fois y a des erreurs, mais tant pis...)
        id3(this.files[0], function(err, tags) {
            console.log(tags);
            document.getElementById("titre").value = tags["title"].replace(/\0[\s\S]*$/g,''); // enleve le \u0000
            document.getElementById("artiste").value = tags["artist"].replace(/\0[\s\S]*$/g,'');
            document.getElementById("album").value = tags["album"].replace(/\0[\s\S]*$/g,'');
        })
}
document.getElementById('fichier').addEventListener('change',readFile,false);
            </script>
    </html>
<?php
}
